...
const requestSchema=z.object({restaurantId:z.string().uuid().optional(),period:z.object({from:z.iso.date(),to:z.iso.date()}),metrics:z.record(z.string(),z.unknown())});
analytics.post('/insights',async(req,res)=>{
  const parsed=requestSchema.safeParse(req.body);if(!parsed.success)return res.status(400).json({error:'Invalid analytics request'});if(parsed.data.restaurantId&&!canAccessRestaurant(req.user!,parsed.data.restaurantId))return res.status(403).json({error:'Forbidden'});if(!parsed.data.restaurantId&&req.user!.role==='manager')return res.status(403).json({error:'Forbidden'});if(!config.HF_TOKEN)return res.status(503).json({error:'AI analysis is not configured'});
  const response=await fetch(config.HF_API_URL,{method:'POST',headers:{Authorization:`Bearer ${config.HF_TOKEN}`,'Content-Type':'application/json'},body:JSON.stringify({model:config.HF_MODEL,temperature:0.2,max_tokens:700,messages:[{role:'system',content:'You analyze restaurant operations. Return concise operational insights based only on supplied metrics. Never invent data. Distinguish observations from recommendations. Output valid JSON.'},{role:'user',content:JSON.stringify(parsed.data)}],response_format:{type:'json_schema',json_schema:{name:'operations_insights',strict:true,schema:{type:'object',additionalProperties:false,properties:{summary:{type:'string'},alerts:{type:'array',items:{type:'string'}},opportunities:{type:'array',items:{type:'string'}},recommended_actions:{type:'array',items:{type:'string'}},confidence_note:{type:'string'}},required:['summary','alerts','opportunities','recommended_actions','confidence_note']}}}})});
  if(!response.ok)return res.status(502).json({error:'AI analysis unavailable'});const payload=await response.json() as any;const content=payload.choices?.[0]?.message?.content;try{const insight=typeof content==='string'?JSON.parse(content):content;await query("INSERT INTO audit_logs(actor_id,action,target_type,metadata) VALUES($1,'analytics_generated','analytics',$2)",[req.user!.id,{model:config.HF_MODEL,period:parsed.data.period,restaurantId:parsed.data.restaurantId||null}]);res.json({model:config.HF_MODEL,generatedAt:new Date().toISOString(),insight});}catch{return res.status(502).json({error:'Invalid AI analysis response'});}
});
